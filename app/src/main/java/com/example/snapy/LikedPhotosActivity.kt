...
class LikedPhotosActivity : AppCompatActivity() {
    ...
    private val likedPhotos get() = PhotoSwipeActivity.likedPhotos
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        undoFab = findViewById(R.id.fabUndo)
        shareFab = findViewById(R.id.fabShare)
        emptyStateText = findViewById(R.id.emptyStateText)

        // Show only liked buttons, hide disliked buttons
        findViewById<android.view.View>(R.id.likedButtonsLayout).visibility = android.view.View.VISIBLE
        findViewById<android.view.View>(R.id.dislikedButtonsLayout).visibility = android.view.View.GONE
        ...
        shareFab.setOnClickListener {
            if (likedPhotos.isNotEmpty()) {
                val intent = Intent(Intent.ACTION_SEND_MULTIPLE).apply {
                    type = "image/*"
                    putParcelableArrayListExtra(Intent.EXTRA_STREAM, ArrayList(likedPhotos.mapNotNull { it.imageUri }))
                }
                startActivity(Intent.createChooser(intent, "Share liked photos"))
            } else Snackbar.make(recyclerView, "No photos to share", Snackbar.LENGTH_SHORT).show()
        }
    }
    ...
    private fun undoLastLikedPhoto() {
        if (likedPhotos.isNotEmpty()) {
            likedPhotos.removeAt(likedPhotos.size - 1)
            updateAdapter()
            Snackbar.make(recyclerView, "Removed from liked photos", Snackbar.LENGTH_SHORT).show()
        } else {
            Snackbar.make(recyclerView, "No liked photos to undo", Snackbar.LENGTH_SHORT).show()
        }
    }
    ...
    private fun showDeleteConfirmationDialog(photo: Photo) {
        AlertDialog.Builder(this)
            ...
            .setMessage("Are you sure you want to remove this photo from liked photos?")
            .setPositiveButton("Delete") { _, _ ->
                likedPhotos.remove(photo)
                updateAdapter()
                Snackbar.make(recyclerView, "Photo removed from liked photos", Snackbar.LENGTH_SHORT).show()
            }
    ...
    private fun updateAdapter() {
        if (isGridMode) {
            gridAdapter.submitList(likedPhotos.toList())
        } else {
            listAdapter.submitList(likedPhotos.toList())
        }
